...
#'
#' @return the estimated regression coefficient beta
#' @export
#'
#' @examples
...
elnet_coord <- function(x,y,a,lambda,betahat=rep(0,NCOL(x)),maxiteration=100){
  for (i in 1:maxiteration) {
    ...
    for (j in 1:NCOL(x)) {
      ...
      num <- 2*(x[,j]%*%r)/NROW(x)
      denom <- 2*(sum(x[,j]^2)/NROW(x)+(1-a)*lambda)
      if (num>-a*lambda & num<a*lambda){
        beta1[j]=0
      }
      else{
        #solve beta1[j] by equating derivatives of minimizing function to zero. There're 2 cases:
        #1. beta1[j]>0; 2. beta1[j]<0
        beta1[j]=ifelse(num>a*lambda,(num-a*lambda)/denom,(num+a*lambda)/denom)
      }
    }
